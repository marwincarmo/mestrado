...
We conducted a parallel analysis and exploratory factor analysis (with oblimin rotation) to estimate the number of facets underlying the instrument items. Then, we ran a Confirmatory Factor Analysis to obtain fit statistics and parameter estimates. Reliability indices for each of the facets were obtained using Cronbach’s $\alpha$ and McDonald’s $\omega$. We assessed construct validity by examining the relations with variables known to be linked with insomnia.

# Results

Mean age was `r round(mean(ebas_full$idade, na.rm=TRUE), 1)` $\pm$ `r round(sd(ebas_full$idade, na.rm=TRUE), 1)` years. Our sample included `r pull(dplyr::count(ebas_full, sexo)[1,2])` women and `r pull(dplyr::count(ebas_full, insonia)[2,2])` participants with insomnia symptoms. 

The parallel analysis procedure indicated two latent factors: Avoidance (two items) and Distress (four items). The 2-factor model showed a good fit [$\chi^2$(`r fitMeasures(fitCFA.ebas2, "df")`) = `r round(fitMeasures(fitCFA.ebas2, "chisq"),2)`, RMSEA = `r round(fitMeasures(fitCFA.ebas2, "rmsea"),3)` [`r round(fitMeasures(fitCFA.ebas2, "rmsea.ci.lower"),2)`, `r round(fitMeasures(fitCFA.ebas2, "rmsea.ci.upper"),3)`]; CFI = `r round(fitMeasures(fitCFA.ebas2, "cfi"),3)`; RNI = `r round(fitMeasures(fitCFA.ebas2, "rni"),3)`; TLI = `r round(fitMeasures(fitCFA.ebas2, "tli"),3)`] and factor loadings ranging from .55 to .95. 
...
digraph {
  ...
  'x1' -> 'x1':e [dir=both label = '.70']
  'x2' -> 'x2':e [dir=both label = '.48']
  'x3' -> 'x3':e [dir=both label = '.13']
  'x4' -> 'x4':e [dir=both label = '.10']
  'x5' -> 'x5':e [dir=both label = '.28']
  'x6' -> 'x6':e [dir=both label = '.21']
...
Preliminary evidence suggest that the SAS is a valid tool to assess acceptance of sleep problems.
