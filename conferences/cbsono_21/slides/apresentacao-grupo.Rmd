...
A média de idade dos participantes foi de `r round(mean(pull(dados, idade)), 1)` $\pm$ `r round(sd(pull(dados, idade)), 1)` anos. Dos `r nrow(dados)` participantes, `r sum(pull(dados, sexo) == "F")` são do sexo feminino. O valor mediano da renda familiar mensal foi de R$ `r median(dados_renda, na.rm = TRUE)`,00. 
...
```{r mapa1, eval=FALSE, message=FALSE, include=FALSE}
...
```{r histogramas, echo=FALSE, fig.alt="Distribuição das variáveis contínuas"}
